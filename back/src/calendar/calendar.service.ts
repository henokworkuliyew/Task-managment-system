...
export class CalendarService {
  ...
  async createEvent(createEventDto: CreateEventDto, userId: string): Promise<CalendarEvent> {
  ...
  async getEvents(filters: EventFilters, userId: string): Promise<CalendarEvent[]> {
    ...
    if (filters.type && filters.type.length > 0) {
      queryBuilder.andWhere('event.type IN (:...types)', { types: filters.type })
    }

    if (filters.priority && filters.priority.length > 0) {
      queryBuilder.andWhere('event.priority IN (:...priorities)', { priorities: filters.priority })
    }

    if (filters.visibility && filters.visibility.length > 0) {
      queryBuilder.andWhere('event.visibility IN (:...visibilities)', { visibilities: filters.visibility })
    }

    if (filters.projectId) {
      queryBuilder.andWhere('event.projectId = :projectId', { projectId: filters.projectId })
    }

    if (filters.taskId) {
      queryBuilder.andWhere('event.taskId = :taskId', { taskId: filters.taskId })
    }

    if (filters.createdById) {
      queryBuilder.andWhere('event.createdById = :createdById', { createdById: filters.createdById })
    }
  ...
  async updateEvent(eventId: string, updateEventDto: UpdateEventDto, userId: string): Promise<CalendarEvent> {
    ...
    if (updateEventDto.projectId && updateEventDto.projectId !== event.projectId) {
      const project = await this.projectRepository.findOne({
  ...
  async getEventsByProject(projectId: string, userId: string): Promise<CalendarEvent[]> {
    return this.getEvents({ projectId }, userId)
  }

  async getEventsByTask(taskId: string, userId: string): Promise<CalendarEvent[]> {
    return this.getEvents({ taskId }, userId)
  }

  private validateEventDates(eventData: { startDate: Date; endDate?: Date; allDay?: boolean }): void {
    ...
    if (endDate) {
      ...
      if (allDay) {
        ...
        if (endDateOnly < nextDay) {
          throw new BadRequestException('All-day events must have end date at least one day after start date')
        }
      }
    } else if (!allDay) {
